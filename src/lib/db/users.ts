...
export async function createOrUpdateUserSubscription(
  ...
  try {
    ...
    if (data.subscriptionStartDate instanceof Date) {
      dataWithTimestamps.subscriptionStartDate = Timestamp.fromDate(
        data.subscriptionStartDate
      );
    }
    if (data.subscriptionEndDate instanceof Date) {
      dataWithTimestamps.subscriptionEndDate = Timestamp.fromDate(
        data.subscriptionEndDate
      );
    }
    if (data.lastPaymentDate instanceof Date) {
      dataWithTimestamps.lastPaymentDate = Timestamp.fromDate(
