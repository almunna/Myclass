...
export default function ReportsPage() {
  ...
  // Chart data states
  const [studentExitsChartData, setStudentExitsChartData] = useState<ChartData[]>([]);
  ...
  // Chart colors
  const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#A28BFF', '#FF6B6B', '#4ECDC4', '#FD7272'];
  ...
  const fetchRoomExits = async () => {
    ...
    try {
      ...
      // Sort by exit time
      exitsList.sort((a, b) => {
        const dateA = a.exitTime?.toDate ? a.exitTime.toDate() : new Date(a.exitTime);
        const dateB = b.exitTime?.toDate ? b.exitTime.toDate() : new Date(b.exitTime);
  ...
  useEffect(() => {
    ...
    if (startDate && endDate) {
      ...
      filtered = filtered.filter(exit => {
        const exitDate = exit.exitTime?.toDate ? exit.exitTime.toDate() : new Date(exit.exitTime);
    ...
    if (selectedStudentId !== "all") {
      filtered = filtered.filter(exit => exit.studentId === selectedStudentId);
    }
  ...
  }, [roomExits, startDate, endDate, selectedSchoolYearId, selectedPeriodId, selectedStudentId, periods, schoolYears]);
            ...
            <Card className="mb-6">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                  {/* School Year Filter */}
                  <div>
                    <Label htmlFor="school-year-filter" className="mb-2 block">School Year</Label>
                  ...
                  {/* Period Filter */}
                  <div>
                    <Label htmlFor="period-filter" className="mb-2 block">Filter by Period</Label>
                  ...
                  <div>
                    <Label htmlFor="student-filter" className="mb-2 block">Filter by Student</Label>
                ...
                <Card>
                  ...
                  <CardContent>
                    {summaryData.length === 0 ? (
                      <p className="text-muted-foreground text-center py-6">No data available for the selected filters</p>
                    ) : (
                          ...
                          <TableBody>
                            {summaryData.map((row) => (
                              <TableRow key={row.studentId}>
                                ...
                                <TableCell>{row.periodName || "No Period"}</TableCell>
                      ...
                      <>
                        ...
                        <div className="flex items-center justify-between space-x-2 py-4">
                          <div className="text-sm text-muted-foreground">
                            Showing <span className="font-medium">{indexOfFirstExit + 1}</span> to{" "}
                            <span className="font-medium">{Math.min(indexOfLastExit, filteredExits.length)}</span> of{" "}
                            <span className="font-medium">{filteredExits.length}</span> entries
                          </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>
                          Students with the most room exits during the selected period
                        </CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="h-[300px] mt-4">
                          <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                              ...
                              <Tooltip />
                              <Legend />
                              <Bar dataKey="value" name="Number of Exits" fill="#8884d8" />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      </CardContent>
                        ...
                        <div className="h-[300px] mt-4">
                          <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                              ...
                              <Legend />
                              <Bar dataKey="value" name="Minutes Out of Class" fill="#FF8042" />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      </CardContent>
                          ...
                          <div className="h-[300px] w-full lg:w-1/2">
                            <ResponsiveContainer width="100%" height="100%">
                              ...
                              >
                                ...
                                <Tooltip />
                                <Bar dataKey="value" name="Number of Exits" fill="#82ca9d" />
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          ...
                          <div>
                            <h3 className="text-sm font-semibold mb-4">Time of Day Distribution</h3>
                            <div className="h-[250px]">
                              <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                  ...
                                  <YAxis allowDecimals={false} />
                                  <Tooltip />
                                  <Bar dataKey="exits" name="Room Exits" fill="#8884d8" />
                                </BarChart>
                              </ResponsiveContainer>
                            </div>
                          </div>
                          ...
                          <div>
                            <h3 className="text-sm font-semibold mb-4">Day of Week Distribution</h3>
                            <div className="h-[250px]">
                              <ResponsiveContainer width="100%" height="100%">
                                <BarChart
                                  data={dayDistributionData.sort((a, b) => a.day - b.day)}
                                  margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
                                >
                                  <CartesianGrid strokeDasharray="3 3" />
                                  <XAxis dataKey="name" />
                                  <YAxis allowDecimals={false} />
                                  <Tooltip />
                                  <Bar dataKey="exits" name="Room Exits" fill="#82ca9d" />
                                </BarChart>
                              </ResponsiveContainer>
                            </div>
                          </div>
